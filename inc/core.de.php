...
if (function_exists('register_nav_menus')) {
    register_nav_menus(array(
        'header_nav' => __('Top'),
        'footer_nav' => __('Bottom'),
        'social_nav' => __('Social')
    ));
}
...
function theme_customize_register($wp_customize)
{
    ...
    $wp_customize->add_control('biji_setting_thumb', array(
        'label' => 'Turn on article thumbnail',
        'section' => 'biji_setting',
        'type' => 'checkbox'
    ));
...
if (!function_exists('pagenavi')) {
    function pagenavi($p = 5)
    {
        ...
        if ($paged > 1) p_link($paged - 1, 'Pre', '«');
        if ($paged > $p + 1) p_link(1, '初め');
        if ($paged > $p + 2) echo '... ';
        for ($i = $paged - $p; $i <= $paged + $p; $i++) {
            if ($i > 0 && $i <= $max_page) $i == $paged ? print "<span class='page-numbers current'>{$i}</span> " : p_link($i);
        }
        if ($paged < $max_page - $p - 1) echo '... ';
        if ($paged < $max_page - $p) p_link($max_page, '最後');
        if ($paged < $max_page) p_link($paged + 1, 'Next', '»');
    }

    function p_link($i, $title = '', $linktype = '')
    {
        if ($title == '') $title = "第 {$i} ページ";
        if ($linktype == '') {
            $linktext = $i;
        } else {
            $linktext = $linktype;
